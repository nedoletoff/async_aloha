...
class AsyncSystem:
    ...
    def step(self):
        ...
        if len(self.queue) > 0:
            if self.time_now < self.queue[0]:
                self.time_now = self.queue[0]
                self.t.append(self.mu)
                self.n += 1
            else:
                self.time_now += self.mu
            self.t.append(self.time_now - self.queue.pop(0))
            self.n += 1
            return
        self.time_now += self.mu
...
if __name__ == '__main__':
    s = AsyncSystem(mu=1, lam=0.4, time_end=10000)
    t, n_av = s.run()
    d_t = 0
    for i in t:
        d_t += i
